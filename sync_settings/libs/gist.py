...
class Gist:
    ...
    def __do_request(self, verb, url, **kwargs):
        ...
        try:
            ...
            raise NetworkError(
                "Can`t perform this action due to network errors. reason: {}".format(
                    str(e)
                )
            )
        ...
        if response.status_code == 404:
            raise NotFoundError(
                "The requested gist do not exists, or the token has not enough permissions"
            )
        ...
        if response.status_code >= 300:
            raise UnexpectedError(
                "Unexpected Error, Reason: {}".format(response.json()["message"])
            )
